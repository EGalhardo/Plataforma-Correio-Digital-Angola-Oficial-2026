...
// ============================================================================
// CDA SECURE LAYER — Módulo Criptográfico AES-256-GCM Backend (Serverless Parity)
// ----------------------------------------------------------------------------
...
const SUFIXO_DETALHE = '__detalhe';
const chaveDetalhe = (chave: string): string => `${chave}${SUFIXO_DETALHE}`;
const ehChaveDetalhe = (chave: string): boolean => String(chave || '').endsWith(SUFIXO_DETALHE);
/** Chaves do guião efectivamente recolhidas (ignora os detalhes). */
const chavesRecolhidas = (campos: Record<string, string> | null | undefined): string[] =>
...
const agregarCamposRespostas = (linhas: { campos: Record<string, string> | null | undefined }[]): AgregadoInqueritoIA[] => {
...
"Categoria": { um: "Ovikalo", ki: "Mbandu", kk: "Nsamu", ch: "Chisinde", ng: "Mutende", kw: "Ombila", nh: "Okanda", fi: "Bumboti" },
    ...
    if (url.includes('/api/inquerito-ia')) {
      const temas = String(body?.temas || '').trim().slice(0, 1500);
      const informacoes = String(body?.informacoes || '').trim().slice(0, 1500);
      const instituicao = String(body?.instituicao || '').trim().slice(0, 120);
      ...
      if (ai) {
        for (const modelo of ["gemini-3.6-flash", "gemini-3.5-flash"]) {
          try {
            ...
            if (s) return res.status(200).json({ ok: true, modelo, sondagem: s });
          } catch (geminiErr) {
            console.error(`Gemini inquerito-ia (${modelo}) erro:`, (geminiErr as Error)?.message?.slice(0, 160));
          }
        }
      }
      ...
      if (groq) {
        try {
          ...
          if (s) return res.status(200).json({ ok: true, modelo: "openai/gpt-oss-120b", sondagem: s });
        } catch (groqErr) {
  ...
  return dadosExecutarPedido({ supaUrl: opts.supaUrl, serviceKey: opts.serviceKey, body: opts.body, authorization: auth });
    ...
    if (url.includes('/api/limpar-orfaos') && method === 'POST') {
      try {
        ...
        try { storageOrfaos = await varrerStorageOrfaosRest(supaUrlPerfil, serviceKeyPerfil); } catch { /* melhor esforço */ }
        return res.status(200).json({ ok: true, analisadas: todas.length, orfas: orfas.length, apagadas, anexosPurgados, ...storageOrfaos });
      } catch (e: any) {
    ...
    if (url.includes('/api/enviar-email-boas-vindas') && method === 'POST') {
      try {
        const { nome, email, chaveAcesso, senha, area, link } = body || {};
...
<div style="max-width:560px;margin:24px auto;background:#ffffff;border-radius:24px;overflow:hidden;border:1px solid #e2e8f0">
  ...
  <div style="background:#f8fafc;padding:16px 32px;color:#94a3b8;font-size:10px;text-align:center">Correio Digital Angola — República de Angola</div>
        ...
        const r = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({ from: de, to: [para], subject: 'Bem-vindo(a) à plataforma Correio Digital Angola — os seus dados de acesso', html }),
        });
        if (!r.ok) {
...
<div style="max-width:560px;margin:24px auto;background:#ffffff;border-radius:24px;overflow:hidden;border:1px solid #e2e8f0">
  ...
  <div style="background:#f8fafc;padding:16px 32px;color:#94a3b8;font-size:10px;text-align:center">Correio Digital Angola — República de Angola</div>
    ...
    // CDA SECURE LAYER: Verificação de Estado Criptográfico (Espelho 1:1 server.ts)
    if (url.includes('/api/crypto/status') && method === 'GET') {
      try {
    ...
    // CDA SECURE LAYER: Cifragem de campos/payloads sensíveis
    if (url.includes('/api/crypto/encrypt') && method === 'POST') {
      try {
        const { text, payload, fields } = body || {};
    ...
    // CDA SECURE LAYER: Decifragem de campos/payloads sensíveis
    if (url.includes('/api/crypto/decrypt') && method === 'POST') {
      try {
        const { ciphertext, payload, fields } = body || {};
