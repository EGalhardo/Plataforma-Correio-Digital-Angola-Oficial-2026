...
## v37.8 — PERSISTÊNCIA DE LEITURA + PAINEL SÓ-CÓDIGO + REGISTO POR COMPARAÇÃO IA DO B.I. (2026-08-24)

Requisitos do proprietário executados:
0. Correspondência lida uma vez nunca volta a «Não Lidas» após terminar sessão e reentrar.
1. Painel institucional (área sob «Área Institucional») exibe APENAS o código da instituição
   (p.ex. `INAPEM-LLMM`), sem qualquer outro dado ou informação.
2. Registo do cidadão sem etapa de registo/verificação facial: a IA compara apenas os dados
   extraídos do B.I. com os dados do formulário (nome, nº B.I., data de nascimento, sexo).
3. Dados coincidentes → popup «Aprovado / Seu cadastro foi aprovado.», página de registo fecha
   sozinha e redirecciona para o Login; login com as credenciais criadas funciona.
4. Aprovação automática: Homologação passa a «Aprovado» imediatamente (sem aprovação manual)
   e o cidadão fica autorizado em todas as funcionalidades.
5. Divergências → cadastro NÃO aprovado, cidadão informado dos dados a corrigir/verificar,
   podendo corrigir e repetir a validação.

Alterações:
- `src/App.tsx`: efeito de reaplicação do estado «lida» agora reage também a `triggerRefetch`
  e às quatro caixas de entrada (correio + documentos, cidadão + instituição), garantindo que o
  estado de leitura persiste imediatamente após cada re-sincronização; o Painel passa
  `instCodigo` ao HomeContent.
- `src/components/features/HomeContent.tsx`: sessão institucional com código resolve APENAS o
  código em monospace centrado (nada mais é renderizado nessa área).
- `src/components/features/RegisterStepper.tsx`:
  · passo 2 do cidadão ganha «Data de Nascimento» e «Sexo»;
  · passo 3 do cidadão substituído por «Validação Automática por IA» (resumo + resultado);
    câmara/biometria facial mantidas APENAS para instituições;
  · pedido de pré-verificação envia `dataNascimento` e `sexo`;
  · veredicto ≠ APTO (cidadão): nenhum cadastro criado, aviso «Dados não correspondem» com o
    motivo + divergências, botão «Voltar» para corrigir e repetir;
  · veredicto APTO (cidadão): score 100 «IA — comparação B.I./formulário», Homologação
    «Aprovado» imediata e popup «Aprovado» → redireccionamento automático para o Login (6 s ou
    clique em «Entendi»).
- `scripts/e2e_paginas.mjs`: Painel institucional agora verificado como «só código» (a liga
  «Pagamentos» do painel foi removida pela regra do proprietário; a página de cobranças continua
  acessível via Assistente de IA).

Testes (browser real, build de produção local :3100):
- `/tmp/test_v378.mjs` — 17/17 asserções PASS, 0 erros JS:
  T1–T3 leitura persiste entre sessões (Não Lidas→abrir→Sair→entrar→apenas em «Lidas»);
  T4–T5 Painel só com código institucional;
  T6–T10 registo cidadão sem câmara, painel «Validação Automática por IA», veredicto REVISAO
  simulado ⇒ aviso «Dados não correspondem», volta à Identidade, nenhum registo criado;
  T11–T15 veredicto APTO simulado ⇒ popup «Aprovado / Seu cadastro foi aprovado.», redirect ao
  Login, fila de homologação com «Aprovado», login com as credenciais criadas funciona.
- Varreduras completas: demo 49 PASS / 0 WARN / 0 FAIL; contas reais 39 PASS / 0 WARN / 0 FAIL.
- `tsc --noEmit`: 0 erros. Build de produção: OK.
